...
<div class="panel panel-default agendamentos">
   ...
   $now = Time::now();
   $hoje = $now->format('Ymd');
   $data = $agendamento->data->format('Ymd');
   
   $situacoes = $situacoes->toArray();
   
  	if($data > $hoje ) {

		unset($situacoes[1]);	// Remove a opção "REALIZADA" para cirurgias que ainda vão acontecer.
   	}
...
// debug($agendamento);
   ...
   <div class="panel-body" style="position: relative;">
		...
		<div class="col-md-2 ">
		<?php 
		// Se estiver na situação: AGENDADA E REALIZADA, permite alterar.
		if(in_array($agendamento->situacao_id,[1,2] ))
			echo $this->Form->input('situacao_id', ['empty'=>' ','style'=>'width:100%','required'=>'required','label'=>'Situação','options' => $situacoes, 'onchange'=>'mostraOpcoes(this)']);
		else
			echo $this->Form->input('situacao_id', ['readOnly'=>'readOnly','disabled'=>'disabled','style'=>'width:100%','required'=>'required','label'=>'Situação','options' => $situacoes]);
		?>
		
        	
        </div>
        	<div class="col-md-5 hide" id="novaData">
		<?php   echo $this->Form->input('nova_data',['id'=>'NovaDataId','style'=>'width:100%','required'=>'required','empty'=>' ','label'=>'Remarcar (nova data):', 'options'=>$vagas, 'class'=>'select2']); ?> 
		</div>
		...
		<div class="col-md-2	">
        	...
        	<?php 
        	//if($bloqueia_data)
        	//	echo "<h3><br/>".$agendamento->data->format('d/m/Y')."</h3>"; //$this->Form->input('data', ['value'=>$agendamento->data->format('d/m/Y'), 'label' => 'Data', 'type' => 'text','class'=>'disabled', 'append'=>$this->Html->icon('calendar'), 'readOnly'=>'readOnly']);
        	//else
        		echo $this->Form->input('data', ['readOnly'=>'readOnly','disabled'=>'disabled','id'=>'DataId','value'=>$agendamento->data->format('d/m/Y'),'label' => 'Data', 'type' => 'text', 'append'=>$this->Html->icon('calendar')]); 
        	
        	?>
        </div>
		<div class="col-md-2">
         <?php echo $this->Form->input('horario', ['readOnly'=>'readOnly','disabled'=>'disabled','label' => 'Horário','value'=>$agendamento->horario->format('H:i:s'), 'type' => 'text', 'append'=>$this->Html->icon('time')]); ?>
        </div>
        <div class="col-md-2">
        <?php   echo $this->Form->input('sala_id', ['readOnly'=>'readOnly','disabled'=>'disabled','label'=>'Sala','options' => $salas]); ?>
 		</div>
		<div class="col-md-2">
        <?php echo $this->Form->input('duracao', ['label' => 'Duração','value'=>$agendamento->duracao->format('H:i:s'), 'class'=>'time', 'type' => 'text', 'append'=>$this->Html->icon('time')]); ?>
        </div>
			...
			<table class="table  table-bordered table-condensed table-hover" id="tab_logic">
				...
				<tbody>
					<tr id='addr' style="display:none;visibility: false">
						...
						<?php echo $this->Form->input('motivo', ['class'=>'motivo','multiple'=>'multiple','style'=>'width:100%','id'=>'motivo','label'=> false, 'options'=>$motivos,'empty'=>' ']) ?>
					...
					<tr id='addr<?php echo $i; ?>' style="">
						...
						<td width="300px">
						<?php echo $this->Form->input('cirurgias_procedimentos.'.$i.'.id', ['value'=>$proc->id,'label'=> false,'type'=>'hidden' ]) ?>
						<?php echo $this->Form->input('cirurgias_procedimentos.'.$i.'.agendamento_id', ['value'=>$proc->agendamento_id,'label'=> false,'type'=>'hidden' ]) ?>
					   
						<?php echo $this->Form->input('cirurgias_procedimentos.'.$i.'.procedimento_id', ['id'=>'cirurgias_procedimentos.'.$i.'.procedimento_id','value'=>$proc->procedimento_id,'required'=>'required', 'style'=>'width:100%','id'=>'procedimento','label'=> false, 'options'=>$procedimentos,'class'=>'select2 procedimento']) ?>
						...
						<?php echo $this->Form->input('cirurgias_procedimentos.'.$i.'.motivos._ids', ['id'=>'cirurgias_procedimentos.'.$i.'.motivos._ids','value'=>$proc->motivo_id, 'placeholder'=>'Motivos','multiple'=>'multiple','style'=>'width:100%','id'=>'motivo','label'=> false, 'options'=>$motivos,'empty'=>'Motivo ', 'class'=>'select2 motivo']) ?>
...
// 			console.log(proced);
	...
	function mostraOpcoes(obj){
		if(obj.value == 5){
			$('#novaData').removeClass('hide');
		}
		else {
			$('#novaData').addClass('hide');
		}
