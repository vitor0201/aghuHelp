...
class FilterComponent extends Component
{
	...
	public $pagination_key = ['_','sort','direction', 'page', 'lang', 'limit'];
	...
	private function getFilter($field, $value, $filter){
		...
		if(in_array($operator, ['LIKE', 'NOT LIKE', 'ILIKE'])){
				
			if($explode) {
				$value_exploded = explode(' ', $value);
				$OR_AND = [ "$explode" =>[] ];
				foreach ($value_exploded as $vl) {
					$OR_AND[$explode][] = array("$field $operator" => "$before$vl$after");
				}
				return $OR_AND;
	
			}
			return array("$field $operator" => "$before$value$after");
		}
		// se for =...
	...
	public function getConditions( $save=[] ) {
		...
		foreach ($this->the_controller->request->data as $filter => $value){
			...
			if(array_key_exists($filter,$this->filters )){  // se o filtro existe
				//debug($this->hasValue($value));
				//debug($value);
				if($this->hasValue($value)) {   // se o valor foi preenchido
					$field = $this->filters[$filter]['field'];
					// prepara a url
					$operator 	= isset($this->filters[$filter]['operator']) ? $this->filters[$filter]['operator'] : '=';
					$this->_url[$filter] = in_array($operator,['IN', 'NOT IN']) ? implode(',',$value) : $value;
...
// 		debug($conditions);
		...
		foreach ($this->the_controller->request->query as $qr_filter => $value){
			...
			if(array_key_exists($qr_filter,$this->filters )){  // se o filtro existe
				...
				if($this->hasValue($value)) {   // se o valor foi preenchido
					...
					$this->the_controller->request->data[$qr_filter] = in_array($operator,['IN', 'NOT IN']) ? explode(',',$value) : $value;   // mantem data preenchido...
					$this->the_controller->request->query[$qr_filter] = in_array($operator,['IN', 'NOT IN']) ? explode(',',$value) : $value;
	...
	public function getUrl(){
		//debug($this->the_controller->request->query);
		return $this->_url;
	}
