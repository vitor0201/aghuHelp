...
class EstatisticasController extends AppController
{
	...
	public function leitos()
	{
		...
		/**
		 * SUBQUERY PARA ENCONTRAR A DATA DO ULTIMO MOVIMENTO
		 */
		$movimentos = TableRegistry::get('Movimentos', [
			'table' => 'agh.ain_movimentos_internacao',
			'connection' => $conn
		]);
		$dtInter = $movimentos->find('all')
		->select(['data_inter' => 'Movimentos.dthr_lancamento'
		])->where(['Movimentos.int_seq = Internacoes.seq AND Movimentos.tmi_seq NOT IN (11, 12, 13, 22)'
		])->order(['Movimentos.dthr_lancamento DESC'
		])->limit(1);
		/*
		 * PACIENTES
		 ...
		 * tml_cod == 16
		 */
		$internacoes = TableRegistry::get('Pacientes', [
			'table' => 'agh.aip_pacientes',
			'connection' => $conn
		]);
		...
		$internacoesQuery = $internacoes->find('all')->select([
			...
			'unidade' => "COALESCE(unidade.seq, leitos.unf_seq)",
		...
		$leitos = $leitos->find('all')->select([
			...
			'Internacoes.lto_lto_id =  Leitos.lto_id AND Internacoes.dt_saida_paciente is null'
		])->where([
